...
#        print("moving data")
        ...
        print('Fitting model...')
#        history=model.fit(imgs_train, imgs_mask_train, batch_size=2, epochs=200, verbose=1,
#                  validation_data=(imgs_val, imgs_mask_val), shuffle=True, callbacks=[model_checkpoint,reducelearning,earlystop])
        history = model.fit_generator(train_dataloader, epochs=100, 
                          steps_per_epoch=len(train_dataloader),
                          validation_data=valid_dataloader, 
                          validation_steps=len(valid_dataloader),
                          callbacks=[model_checkpoint,reducelearning,earlystop])
        yaml_string = model.to_yaml()
        with open(result_dir+'/'+data+'_'+test_type+'.yaml', "w") as yaml_file:
        ...
        plt.legend()
        fig=plt.gcf()
        plt.savefig(result_dir+'/accuracy_'+data+'_'+test_type+'.png',dpi=100)
        ...
        fig=plt.gcf()
        plt.savefig(result_dir+'/loss_'+data+'_'+test_type+'.png',dpi=100)
        plt.close('all')
