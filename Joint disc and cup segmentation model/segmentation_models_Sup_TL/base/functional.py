...
def general_dice(gt, pr, class_weights=1, class_indexes=None, smooth=SMOOTH, per_image=False, threshold=None,
            **kwargs):
    r"""The F-score (Dice coefficient) can be interpreted as a weighted average of the precision and recall,
    where an F-score reaches its best value at 1 and worst score at 0.
    The relative contribution of ``precision`` and ``recall`` to the F1-score are equal.
    ...
    # calculate score
    gtw = 1/(backend.sum(gt, axis=axes)** 2+0.000001)
    numerator=gtw*backend.sum(gt * pr, axis=axes)
    numerator=backend.sum(numerator)
    denominator=gtw*backend.sum(gt + pr, axis=axes)
    denominator=backend.sum(denominator)
...
def tversky(gt, pr,alpha=1 ,beta=1, class_weights=1, class_indexes=None, smooth=SMOOTH, per_image=False, threshold=None,
            **kwargs):
    r"""The F-score (Dice coefficient) can be interpreted as a weighted average of the precision and recall,
    where an F-score reaches its best value at 1 and worst score at 0.
    The relative contribution of ``precision`` and ``recall`` to the F1-score are equal.
    ...
    score = (tp + smooth) \
            / (tp + beta* fn + alpha*fp + smooth)
    score = average(score, per_image, class_weights, **kwargs)

    return score

def focaltversky(gt, pr,alpha=1 ,beta=1, class_weights=1, gamma=0.75, class_indexes=None, smooth=SMOOTH, per_image=False, threshold=None,
            **kwargs):
    r"""The F-score (Dice coefficient) can be interpreted as a weighted average of the precision and recall,
    where an F-score reaches its best value at 1 and worst score at 0.
    The relative contribution of ``precision`` and ``recall`` to the F1-score are equal.
    ...
    score = (tp + smooth) \
            / (tp + alpha* fn + beta*fp + smooth)
    score= backend.pow((1 - score), gamma)
...
#   Loss Functions
# ----------------------------------------------------------------
def log_dice(gt, pr, class_weights=1., class_indexes=None, **kwargs):
    
    backend = kwargs['backend']
    per_image=False
    gt, pr = gather_channels(gt, pr, indexes=class_indexes, **kwargs)
    pr = round_if_needed(pr, threshold=None, **kwargs)
    axes = get_reduce_axes(per_image, **kwargs)
    smooth=SMOOTH
    # calculate score
    tp = backend.sum(gt * pr, axis=axes)
    ...
    score = average(score, per_image, class_weights, **kwargs)
    # calculate loss
    output = backend.log(score)
    return - backend.mean(output)

def log_dice2(gt, pr, class_weights=1., class_indexes=None, **kwargs):
    ...
    score = ((1 + beta ** 2) * tp + smooth) \
            / ((1 + beta ** 2) * tp + beta ** 2 * fn + fp + smooth)
    score = backend.log(score)
    score = -average(score, per_image, class_weights, **kwargs)
    # calculate loss
    return score

def log_tversky(gt, pr,alpha=1 ,beta=1, class_weights=1, class_indexes=None, smooth=SMOOTH, per_image=False, threshold=None,
            **kwargs):
...
def suppressed_tversky_loss(gt, pr,alpha=1 ,beta=1, class_weights=1., gamma=2.0,k=0.25, class_indexes=None,threshold=None,smooth=SMOOTH, per_image=False, **kwargs):
    r"""Implementation of Focal Loss from the paper in multiclass classification
    ...
    # Calculate focal loss
    score = -(k * backend.pow((1 - score), gamma) * backend.log(score))
    loss = average(score, per_image, class_weights, **kwargs)
